...
def euler(funcao, valor_y, intervalo, h):
  x, y = symbols('x y')
  saida = []

  valor_x = intervalo[0]
  while valor_x < intervalo[1]:
    result_funcao = funcao.subs({x: valor_x, y: valor_y})
    valor_y += h * result_funcao
...
def main():
  ...
  with open("saidas.txt", "w") as arq:
    for i in range(len(funcao)):
      resultado = euler(funcao[i], y_inicial[i], intervalo[i], h[i])

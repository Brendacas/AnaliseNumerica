...
def euler_modificado(funcao, valor_y, intervalo, h):
    x, y = symbols('x y')
    saida = []
    ...
    while valor_x < intervalo[1]:
        ...
        y_predito = valor_y + h * gradiente_inicial

        gradiente_final = funcao.subs({x: valor_x + h, y: y_predito})
        valor_y = valor_y + (h / 2) * (gradiente_inicial + gradiente_final)
...
def main():
    ...
    with open("saidas.txt", "w") as arq:
        for i in range(len(funcao)):
            resultado = euler_modificado(funcao[i], y_inicial[i], intervalo[i], h[i])
